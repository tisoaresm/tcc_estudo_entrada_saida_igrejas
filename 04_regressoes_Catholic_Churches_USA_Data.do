...
use grpcode year fipsmerg cntynm stateab pop_em_milhoes grpname adherent congreg reltrad family a_imt a_imt_1 prev_year n_E n_M n_C ano_mercado Evangelical Mainline Catholic using "Bases_Tratadas\Catholic_Churches_USA_Data.dta"
...
if _rc == 0 {
    outreg2 using "Outputs_Regressoes\Regression_reghdfe_Model1_Catholic_Churches_USA_Data.txt", replace
} 
else {
...
* Salvar os resultados em um arquivo de texto
if _rc == 0 {
    outreg2 using "Outputs_Regressoes\Regression_reghdfe_Model2_Catholic_Churches_USA_Data.txt", replace
} 
else {
...
* Salvar os resultados em um arquivo de texto
if _rc == 0 {
    outreg2 using "Outputs_Regressoes\Regression_reghdfe_Model3_Catholic_Churches_USA_Data.txt", replace
} 
else {
...
* Salvar os resultados em um arquivo de texto
if _rc == 0 {
    outreg2 using "Outputs_Regressoes\Regression_reghdfe_Model4_Catholic_Churches_USA_Data.txt", replace
} 
else {
